...
public class Expression {
    ...
    public Expression(String pExprStr) {
    	...
    	while (token != null) {
    		...
    		if (SubOperator.class.isInstance(token)) {
    			token = negationCheck(token, prevToken);
    		}
    		mTokenQueue.enqueue(token);
    		prevToken = token;
    ...
    public Double evaluate() {
    	...
    	while (mTokenQueue.isEmpty() == false) {
    		...
    		if (Operand.class.isInstance(token)) {
    			Operand operand = null;
    			operand = (Operand) token;
    			operandStack.push(operand);
    		}
    		else if (LeftParen.class.isInstance(token)) {
    			LeftParen leftParen = new LeftParen();
    			operatorStack.push(leftParen);
    		}
    		else if (RightParen.class.isInstance(token)) {
    			while (!LeftParen.class.isInstance(operatorStack.peek())) {
    ...
    private Token negationCheck(Token pToken, Token pPrevToken) {
        if (pPrevToken == null || BinaryOperator.class.isInstance(pPrevToken) || LeftParen.class.isInstance(pPrevToken)) {
            pToken = null;
    ...
    private void topEval(Stack<Operator> pOperatorStack, Stack<Operand> pOperandStack) {
    	...
    	if (UnaryOperator.class.isInstance(operator)) {
			Operand res = ((UnaryOperator) operator).evaluate(right);
			pOperandStack.push(res);
		}
    	else {
    		Operand left = pOperandStack.pop();
    		Operand resu = ((BinaryOperator) operator).evaluate(left,right);
    		pOperandStack.push(resu);
    	}
    }
}
