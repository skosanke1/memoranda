...
//
// DESCRIPTION
// This is the main file that is called to run in the proj3 package
//
// AUTHOR
//Steven Kosanke
//skosanke
 ...
 * the roster to the output file "gradebook.dat". Then we exit the program with a code of 0.
     ...
     */
    public void exit() {
    	GradebookWriter gbWriter = null;
    	try {
    		gbWriter = new GradebookWriter("gradebook.dat");
    		gbWriter.writeGradebook(getRoster());
    		
    	} catch(FileNotFoundException bExcept) {
    ...
    public void run() {
    	...
    	try { 
    		String fName = "gradebook.dat";
    		GradebookReader gbReader = new GradebookReader(fName);
    		setRoster(gbReader.readRoster());
    		    				
		} catch (FileNotFoundException aExcept) {
