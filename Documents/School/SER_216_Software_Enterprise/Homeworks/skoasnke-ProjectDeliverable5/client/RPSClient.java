...
public class RPSClient extends Application 
  ...
  private void connectToServer() {
    ...
    new Thread(() -> {
      try {
        ...
        // Am I player 1 or 2?
        if (player == PLAYER1) {
        	PlayerID.setText("You are Player 1");
        	myToken = '1';
        	Score.setText("Waiting for player 2 to join");

          // Wait till player2 has joined
    	  fromServer.readInt();
          Platform.runLater(() -> 
          Score.setText("Both players are ready."));    
 
          Status.setText("The other player has won.");
          myTurn = true;           
        }
        else if (player == PLAYER2) {
        	PlayerID.setText("You are Player 2");
        	myToken = '2';
        ...
        while (continueToPlay) {      
            if (player == PLAYER1) {
            	Status.setText("Your move! Enter 'r' for rock, 'p' for paper, or 's' for scissors.");
  ...
  private void receiveInfoFromServer() throws IOException {
    //Receive game status
	  //int(scorep1*100+scorep2*10+oppmove)
	  int status = fromServer.readInt();
	  int p1score = (status/100) % 10;
	  int p2score = (status/10) % 10;
	  int oppmove = status % 10;
	  ...
	  Score.setText("Player 1: " + p1score + "   Player 2: " + p2score);
 
    if (p1score >=5) {
      // Player 1 won, stop playing
      continueToPlay = false;
      if (myToken == '1') {
        Platform.runLater(() -> Status.setText("Congratulations, you won!"));
      }
      else if (myToken == '2') {
        Platform.runLater(() -> 
          Status.setText("The other player has won."));
